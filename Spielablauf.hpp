...
public:
    ...
    void Spielzug(Spieler, Spieler, Spieler, Spieler, Spielfeld);
    int BestimmeStartfeldIndex(Spieler, Spielfeld);

    bool SpielEnde(Spieler, Spieler, Spieler, Spieler);

    void FigurBewegen(int, int, int, Spieler, Spieler, Spieler, Spieler, Spielfeld);
    ...
    int EigeneFigurAufNeuemFeld(int, std::string, Spieler);                                                   // Gibt den FigurenIndex der Figur zurueck, die auf dem neuen Feld steht. Falls keine dort steht, wird der Index der Figur die bewegt wird zurÅckgegeben.
    std::string UeberSpielfeldendeHinaus(int, int, std::string, Spieler, Spielfeld);                          // Ueberprueft, ob das neue Feld im Array "Spielbrett" drin ist. Wenn nicht, wird das neue Feld korrigiert, ansonsten wird das neue Feld einfahc zurueckgegeben.
